...
import 'package:my_app/src/util/carreras.dart';
import 'package:my_app/src/util/materias.dart';
...
class _EditarPerfilPageState extends State<EditarPerfilPage> {
  ...
  void initState() {
    ...
    // Datos de profesor
    experienciaController = TextEditingController(text: perfil['experiencia'] ?? '');
    carreraProfesionController = TextEditingController(text: perfil['carrera_profesion'] ?? '');
    ...
    semestreController = TextEditingController(text: perfil['semestre']?.toString() ?? '');
    interesesController = TextEditingController(text: perfil['intereses'] ?? '');
    disponibilidadController = TextEditingController(text: perfil['disponibilidad'] ?? '');
  }
  ...
  try {
      ...
      if (imagenSeleccionada != null && extensionImagen != null) {
        nombreArchivo = '${DateTime.now().millisecondsSinceEpoch}.$extensionImagen';
            ...
            child: Column(
              ...
              children: [
                ...
                if (esProfesor) ...[
                  Padding(
                    padding: const EdgeInsets.symmetric(vertical: 8),
                    child: Text('Datos de profesor', style: Constants.textStyleAccentTitle),
                  ),
                  
                  MultiSelectDialogField(
                    items: materias
                        .map((materia) => MultiSelectItem<String>(materia, materia))
                        .toList(),
                  ...
                  Padding(
                    padding: const EdgeInsets.symmetric(vertical: 8),
                    child: Text('Datos de estudiante', style: Constants.textStyleAccentTitle),
                  ),
